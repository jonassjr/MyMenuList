...
export const getUserCurrentPlan = async (userId: string) => {
  ...
  // const menuItems = await prisma.items.count({
  //   where: {
  //     userId,
  //   },
  // })
  ...
  // const currentMenus = menuCount
  // const usage = (currentMenus / availableItems) * 100

  return {
    name: plan.name,
    quota: {
      menus: plan.quota.menus,
      items: plan.quota.items
    }
  }
}